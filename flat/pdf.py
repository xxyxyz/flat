            ...
            if key not in self.cache:
                ...
                fontfile = stream(
                    0,
                    {
                        ...
                        "Subtype"
                        if cff
                        else "Length1": name("CIDFontType0C")
                        if cff
                        else number(len(fontfiledata)),
                    },
                    fontfiledata,
                )
                descriptor = obj(
                ...
                tounicode = stream(
                    0, {"Length": number(len(tounicodedata))}, tounicodedata
                )
    ...
    def image(self, image):
        ...
        if key not in self.images:
            if key not in self.cache:
                ...
                self.cache[key] = _named_resource(
                    "I%d" % len(self.images), stream(0, setup, data)
                )
...
def _page_fixes(bleed, cropmarks, page):
    ...
    if cropmarks:
        ...
        for x0, y0, x1, y1 in lines:
            fragments.append(
                "%s %s m %s %s l S" % (dump(x0), dump(y0), dump(x1), dump(y1))
            )
        fragments.append("Q")
...
def serialize(document, compress, bleed, cropmarks):
    ...
    for page in document.pages:
        ...
        state.reset()
        resources.reset()
        code = "\n".join(item.pdf(page.height, state, resources) for item in page.items)
    ...
    root = obj(0, dictionary({"Type": name("Catalog"), "Pages": pagesreference}))
    info = obj(
        0, dictionary({"Title": string(document.title), "Producer": string("Flat")})
    )
    pages = pagesreference.obj = obj(
        0,
        dictionary(
            {
                "Type": name("Pages"),
