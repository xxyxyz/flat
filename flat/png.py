...
class png(object):
    ...
    def __init__(self, data):
        ...
        self.width, self.height, depth, color, compression, fltr, interlace = r.parse(
            ">LLBBBBB"
        )  # IHDR
        if depth != 8:
    ...
    def decompress(self):
        ...
        content = bytearray(
            decompress(bytes(self.idat()))
        )  # TODO python 3: remove bytearray/bytes
...
def serialize(image, optimized):
    ...
    if optimized:
        ...
        for y in range(image.height):
            ...
            parts.append("\0")
            parts.append(
                bytes(image.data[offset : offset + wn])
    ...
    return "".join(
        (
            ...
            "IHDR",
            ihdr,
            L(crc32(ihdr, crc32("IHDR")) & 0xffffffff),
            L(len(idat)),
            "IDAT",
            idat,
            L(crc32(idat, crc32("IDAT")) & 0xffffffff),
            L(0),
            "IEND",
            L(crc32("IEND") & 0xffffffff),
        )
    )
